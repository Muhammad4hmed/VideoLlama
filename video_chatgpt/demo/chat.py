...
class Chat:
    ...
    def answer(self, state, img_list, temperature, max_new_tokens, first_run):
        ...
        infer_batch = 32
        video_tensor = image_tensor.half().cuda()
        n_chunk = len(video_tensor)

        # Construct prompt
        prompt = state.get_prompt(self.total_frame_num)
        print(prompt)
        ...
        video_features = torch.FloatTensor(n_chunk, 1024).fill_(0).half().cuda()
        ...
        with torch.no_grad():
            for i in range(n_iter):
                ...
                image_forward_outs = self.vision_tower(video_batch, output_hidden_states=True)
                # select_hidden_state_layer = -2  # Same as used in LLaVA
                select_hidden_state = image_forward_outs.pooler_output # hidden_states[select_hidden_state_layer]
        ...
        video_spatio_temporal_features = video_features#.to(dtype = torch.bfloat16)
        ...
        output = multiply_numbers_if_starts_with(output, self.seg_size)
