...
def train():
    ...
    #     model, quant_config_dict, dtype=torch.qint8
    # )
    ...
    #     model.base_model.model.model.layers[i].post_attention_layernorm.requires_grad_(True)
    model.model.layers[0].requires_grad_(True)
    # model.model.layers[30].requires_grad_(True)
    # model.model.cross_attention.requires_grad_(True)
    model.model.layers[31].requires_grad_(True)
    model.lm_head.requires_grad_(True)
    with open('train_arch.txt', 'w') as f:
