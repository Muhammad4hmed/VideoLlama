...
def main():
    ...
    for video_name in tqdm(all_videos):
        # part_selected = video_name.split('/')[-1]
        # numeric_part = part_selected[1:]
        # if int(numeric_part) < 32:
        #     continue
        for cam in tqdm(glob.glob(video_name + '/*')):
            for action in tqdm(glob.glob(cam + '/*.avi')):
                ...
                video_id = action.split('/')[-1].split('.')[0] + '_' + cam.split('/')[
                    -1]  # video_name.split('.')[0]
                ...
                for i in range(n_iter):
                    ...
                    # video_tensor['pixel_values'] = video_tensor['pixel_values'][min_ind:max_ind]
                    video_batch = video_tensor['pixel_values'][min_ind:max_ind].cuda()

                    # video_tensor1 = image_processor(video, return_tensors='pt')
                    ...
                    batch_features = select_hidden_state[:, :1024]  # select_hidden_state[:, 1:]
                ...
                if counter % 1 == 100:  # Save after every 512 videos, update this number as per your requirements
